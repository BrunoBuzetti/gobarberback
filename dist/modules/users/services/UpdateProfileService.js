...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (emailAlreadyUsed && emailAlreadyUsed.id !== user_id) {
      throw new _AppError.default('Email já utilizado por otro usuario.');
    }
    ...
    if (password && !old_password) {
      throw new _AppError.default('Es necesario indicar la senha antigua para insertar uma nova.');
    }
    ...
    if (password && old_password) {
      ...
      if (!checkOldPassword) {
        throw new _AppError.default('Senha antigua no coincide.');
      }
